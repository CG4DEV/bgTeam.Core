...
& dotnet add "$TEMP_PROJECT" package --package-directory "$TOOLS_DIR" Cake.CoreCLR --version 2.1.0
...
if (!(Test-Path $DOTNET_PATH)) 
{
	...
	powershell -NoProfile -ExecutionPolicy unrestricted -Command "&([scriptblock]::Create((Invoke-WebRequest -useb 'https://dot.net/v1/dotnet-install.ps1'))) -Version 2.1.0 -Runtime dotnet -InstallDir $CLI_DIR -NoPath"
}
