namespace Trcont.Ris.DataAccess.Order
{
    ...
    public class GetKPServicesByDocumentIdCmd : ICommand<GetKPServicesByDocumentIdCmdContext, IEnumerable<OrderServicesDto>>
    {
        private readonly IRepository _repository;

        public GetKPServicesByDocumentIdCmd(IRepository repository)
        {
            _repository = repository;
        }

        public IEnumerable<OrderServicesDto> Execute(GetKPServicesByDocumentIdCmdContext context)
        {
            return ExecuteAsync(context).Result;
        }

        public async Task<IEnumerable<OrderServicesDto>> ExecuteAsync(GetKPServicesByDocumentIdCmdContext context)
        {
            return await _repository.GetAllAsync<OrderServicesDto>(
               @"SELECT
                    ...
                    os.ToPointId,
                    placeFrom.IrsGuid AS ToPointGuid,
